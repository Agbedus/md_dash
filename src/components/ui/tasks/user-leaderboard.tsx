    ...
    <div className="mb-10">
      ...
      <div className="flex flex-nowrap overflow-x-auto gap-3 pb-2 scrollbar-hide">
        {userPerformance.map((user, i) => (
          <div 
            key={user.id} 
            className={`flex items-center gap-3 px-3 py-2 lg:px-6 lg:py-4 rounded-xl lg:rounded-2xl border transition-all duration-300 hover:bg-white/[0.03] group flex-shrink-0 min-w-[140px] lg:min-w-0 ${getAwardBg(i)}`}
          >
            <div className="relative">
              {user.avatarUrl ? (
                <Image 
                  ...
                  width={32} 
                  height={32} 
                  className="rounded-lg lg:rounded-xl border-2 border-white/5 group-hover:border-white/5 transition-colors w-8 h-8 lg:w-12 lg:h-12"
                />
              ) : (
              ...
              <div className="absolute -bottom-1 -right-1 p-0.5 bg-zinc-950 rounded-full border border-white/5 shadow-lg">
                {getAwardIcon(i)}
              </div>
            </div>
            <div>

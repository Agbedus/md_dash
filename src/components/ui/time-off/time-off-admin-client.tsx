        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
                <div>
                    ...
                    <p className="text-zinc-400 text-lg">Review and manage team time-off requests.</p>
                </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                <div className="flex items-center gap-2 w-full overflow-x-auto pb-2 scrollbar-hide">
                    <div className="relative flex-1 min-w-[140px] max-w-sm group">
                        <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-amber-400 transition-colors w-3.5 h-3.5" />
                        <input
                            type="text"
                            placeholder="Search by name or reason..."
                            value={searchQuery}
                            onChange={e => setSearchQuery(e.target.value)}
                            className="w-full pl-8 pr-4 h-11 bg-white/[0.03] border border-white/5 rounded-xl focus:outline-none focus:bg-white/[0.06] focus:border-white/5 text-white placeholder:text-zinc-600 transition-all text-sm"
                        />
                    </div>

                    <div className="relative group flex-shrink-0">
                        <div className="h-11 w-36 bg-white/[0.03] border border-white/5 rounded-xl flex items-center pl-3 relative overflow-hidden focus-within:outline-none focus-within:bg-white/[0.06] transition-all">
                            <FiFilter className="text-zinc-500 absolute left-3 top-1/2 -translate-y-1/2 z-10 w-3.5 h-3.5" />
                            <select
                                value={filterStatus}
                                onChange={e => setFilterStatus(e.target.value)}
                                className="bg-transparent border-none pl-8 pr-4 w-full h-full text-zinc-400 cursor-pointer text-[11px] font-bold uppercase tracking-wider appearance-none focus:outline-none"
                            >
                    ...
                    <div className="relative group flex-shrink-0">
                        <div className="h-11 w-36 bg-white/[0.03] border border-white/5 rounded-xl flex items-center pl-3 relative overflow-hidden focus-within:outline-none focus-within:bg-white/[0.06] transition-all">
                            <FiCalendar className="text-zinc-500 absolute left-3 top-1/2 -translate-y-1/2 z-10 w-3.5 h-3.5" />
                            <select
                                value={filterType}
                            ...
                            >
                                ...
                                <option value="leave" className="bg-zinc-900">Leave</option>
                                <option value="off" className="bg-zinc-900">Day Off</option>
                                <option value="sick" className="bg-zinc-900">Sick</option>
                    ...
                    <table className="w-full border-collapse">
                        ...
                        <thead>
                            <tr className="border-b border-white/5 bg-white/[0.03] text-left">
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Employee</th>
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Type</th>
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Period</th>
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Days</th>
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Justification</th>
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Status</th>
                                <th scope="col" className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Requested</th>
                                <th scope="col" className="px-6 py-4 text-right text-[11px] font-bold text-zinc-500 uppercase tracking-wider whitespace-nowrap">Actions</th>
                            </tr>
                        </thead>
                                ...
                                {filteredRequests.length === 0 && (
                                    <tr>
                                        <td colSpan={8} className="px-6 py-16 text-center">
                                            <FiSun className="w-8 h-8 text-zinc-700 mx-auto mb-3" />
                                            <p className="text-zinc-500 text-sm">No time-off requests found.</p>
                                        </td>
                                    </tr>
                                )}
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-3">
                                                    {user?.avatarUrl ? (
                                                        <Image src={user.avatarUrl} alt={user.fullName || ''} width={32} height={32} className="rounded-full object-cover border border-white/5" />
                                                    ) : (
                                                    ...
                                                    <div>
                                                        <Link href={`/users/${req.user_id}`} className="text-sm font-medium text-white hover:text-amber-300 transition-colors">
                                            ...
                                            {/* Period */}
                                            <td className="px-6 py-4 whitespace-nowrap text-xs text-zinc-400 tabular-nums">
                                            ...
                                            {/* Requested */}
                                            <td className="px-6 py-4 whitespace-nowrap text-xs text-zinc-500 tabular-nums">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={actionLoading === req.id}
                                                        className="p-1.5 rounded-lg hover:bg-rose-500/10 text-zinc-500 hover:text-rose-400 transition-all disabled:opacity-50"
                                                        title="Delete"
                                                    >

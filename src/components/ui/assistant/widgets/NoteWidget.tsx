      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-lg bg-white/5 ${typeColor} font-medium whitespace-nowrap`}>
              {note.type}
            </span>
          </div>
          ...
          {note.tags && (
            <div className="flex flex-wrap gap-1 text-xs">
              {note.tags.split(',').map((tag, idx) => (
                <span key={idx} className="px-2 py-0.5 rounded bg-white/5 text-zinc-500">
                  #{tag.trim()}
                </span>
              ))}
            </div>

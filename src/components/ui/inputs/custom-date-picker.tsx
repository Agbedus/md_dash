  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <div
        ...
        className={`w-full bg-zinc-900/50 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all cursor-pointer flex items-center justify-between gap-2 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/5'}`}
      >
        ...
        {formattedValue && !disabled && (
          <button
            type="button"
            onClick={clearValue}
            className="p-1 hover:bg-white/10 rounded-full text-zinc-500 hover:text-zinc-300 transition-colors"
          >
      ...
      {isOpen && (
        <Portal>
          <div
            ...
            className={`z-[9999] bg-zinc-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-100 ${
          ...
          >
            ...
            <div className="p-3">
              <div className="grid grid-cols-7 mb-2">
                {weekDays.map((day) => (
                  <div key={day} className="text-center text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                    {day}
                  </div>
                ))}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${isSelected ? 'bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-500/20' : 'hover:bg-white/10 hover:text-white'}
            ...
            {enableTime && (
              <div className="border-t border-white/10 p-3 bg-white/[0.02]">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center bg-zinc-950/50 border border-white/10 rounded-lg px-2 py-1 flex-1">
                        <input
                        type="number"
                        min={0}
                        max={23}
            ...
            {/* Footer */}
            <div className="p-2 border-t border-white/5 bg-white/5 flex justify-between items-center">
                <button
                    type="button"
                    onClick={() => {
                        const now = new Date();
                ...
                >
                 ...
                 <button
                     ...
                     className="text-xs text-zinc-400 hover:text-white font-medium px-2 py-1 rounded hover:bg-white/10 transition-colors"
                 >
                     Close
                 </button>
            </div>

...
export function ProjectTable({ projects, users, clients, onSelectProject }: ProjectTableProps) {
  ...
  const statusColors = {
    planning: "bg-white/[0.03] text-zinc-400 border-white/5",
    ...
    <div className="w-full space-y-4">
      ...
      <table className="w-full text-left text-sm min-w-[1400px]">
        <thead className="bg-white/[0.03] border-b border-white/5">
          <tr>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Project</th>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider text-center">Progress</th>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Key</th>
            ...
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Owner</th>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Client</th>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Timeline</th>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Budget</th>
            <th className="px-6 py-4 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Time Logged</th>
        ...
        <tbody className="divide-y divide-white/5">
          {projects.map((project) => {
                          ...
                          <tr>
                            <td className="px-4 py-2 w-[200px]">
                              <input
                                ...
                                required
                                className="w-full bg-zinc-900/50 border border-white/5 rounded px-2 py-1 text-white text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                              />
                            </td>
            ...
            return (
              <React.Fragment key={project.id}>
              <tr 
                className={`group hover:bg-white/[0.03] transition-colors items-center cursor-pointer ${expandedIds.includes(project.id) ? 'bg-white/[0.03]' : ''}`}
                onClick={() => router.push(`/projects/${project.id}`)}
              >
                <td className="px-6 py-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-1.5 h-1.5 rounded-full ${statusColors[project.status].split(' ')[1].replace('text-', 'bg-')}`} />
                    <div className="font-medium text-white text-sm tracking-tight truncate group-hover:text-[var(--pastel-indigo)] transition-colors">{project.name}</div>
                  </div>
                </td>
                ...
                <td className="px-6 py-3">
                  {project.key && (
                    <div className="px-2 py-0.5 rounded-md text-[11px] font-medium font-mono text-zinc-500 bg-white/[0.03] border border-white/5 uppercase tracking-wider inline-block">
                      {project.key}
                    </div>
                  )}
                </td>
                ...
                <td className="px-6 py-3">
                  {owner ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-bold text-zinc-400 group-hover:text-zinc-200 transition-colors uppercase tracking-tight">{owner.fullName}</span>
                    </div>
                  ) : '-'}
                </td>
                <td className="px-6 py-3">
                  <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-tight group-hover:text-zinc-200 transition-colors">
                    {client ? client.companyName : <span className="text-zinc-600 italic">No Client</span>}
                  </span>
                </td>
                <td className="px-6 py-3">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-[11px] font-medium text-white/80 uppercase tracking-tight">
                ...
                <td className="px-6 py-3">
                  {project.budget ? (
                    <div className="flex flex-col">
                      <span className="text-xs font-medium text-white tracking-tight uppercase">
                ...
                <td className="px-6 py-3">
                  <div className="flex items-center gap-1.5">
                    <FiClock className="w-3 h-3 text-zinc-600 flex-shrink-0" />
                    <span className="text-xs font-medium text-white tabular-nums">
                      {totalHours > 0 ? `${totalHours.toFixed(1)}h` : <span className="text-zinc-600 italic text-[11px]">-</span>}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-3 text-right">
                  <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <button
                      ...
                      className="p-2 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] text-zinc-400 hover:text-white border border-white/5 transition-all"
                      title="Edit"
                    >
                    ...
                    <div className="p-2 rounded-xl bg-white/[0.03] text-zinc-500">
                    ...
                    <div className="sticky left-0 p-6 pl-12 bg-zinc-900/30 border-y border-white/5 w-full">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <button 
                          ...
                          className="flex items-center gap-1 text-[11px] font-medium text-zinc-400 hover:text-white transition-all px-2 py-1 rounded bg-white/[0.03] hover:bg-white/[0.06]"
                        >
                      ...
                      {addingTaskId === project.id && (
                        <div className="mb-4 p-4 rounded-xl bg-white/[0.03] border border-indigo-500/20 animate-in fade-in slide-in-from-top-2">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                               <div className="md:col-span-2">
                                 <input 
                                   ...
                                   required 
                                   className="w-full bg-zinc-900/50 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/50"
                                 />
                               </div>
                               <div>
                                 <select 
                                   name="priority" 
                                   defaultValue="medium"
                                   className="w-full bg-zinc-900/50 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/50 appearance-none cursor-pointer"
                                 >
                             ...
                             <div className="flex items-center gap-4">
                               <div className="flex-1">
                                 <textarea 
                                   ...
                                   rows={1}
                                   className="w-full bg-zinc-900/50 border border-white/5 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-indigo-500/50 resize-none"
                                 />
                               </div>
                               <div className="flex gap-2">
                                 ...
                                 <button type="button" onClick={() => setAddingTaskId(null)} className="px-3 py-2 rounded-lg bg-white/[0.03] text-zinc-400 hover:text-white transition-all text-sm border border-white/5">
                                   <FiX className="w-4 h-4" />
                                 </button>
                               </div>
                             </div>
                      ...
                      {project.tasks && project.tasks.length > 0 ? (
                        <div className="overflow-x-auto rounded-lg border border-white/5 bg-zinc-900/50">
                            <table className="w-full text-left text-sm min-w-[1000px]">
                                <thead className="bg-white/[0.03] text-zinc-500 font-bold text-[11px] uppercase tracking-wider">
                                    <tr>
                      ...
                      <tr>
                        <td className="px-4 py-2 w-[200px]">
                          <input
                            ...
                            className="w-full bg-zinc-900/50 border border-white/5 rounded px-2 py-1 text-white text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500/50 placeholder:text-zinc-600"
                          />
                        </td>
                        <td className="px-4 py-2 w-[100px]">
                          <input
            ...
            <tr className="sticky bottom-0 z-10">
              <td colSpan={10} className="px-4 py-3 sticky left-0">
                <button
                  ...
                  className="flex items-center gap-2 p-1.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 hover:border-white/5 text-sm text-zinc-400 hover:text-white transition-all duration-200 group w-fit"
                >
